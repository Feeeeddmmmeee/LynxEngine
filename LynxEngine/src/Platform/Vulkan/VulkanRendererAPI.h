...
constexpr std::string_view TEXTURE_DIR = "LynxEngine/textures/";
	...
	class VulkanRendererAPI : public RendererAPI
	{
		...
		private:
			...
			void setupObjects()
			{
				...
				LYNX_ENGINE_DEBUG("Models loaded!");
			}
					...
					const std::vector<tinyobj::material_t> materials, uint32_t mat)
			{
				...
				if(materials.size())
				{
					...
					mesh.material.setPipeline(pipelineManager->get({
								.vertMain="vertMain",
								.fragMain="fragMain",
								.vert="LynxEngine/shaders/texture.spv",
								.frag="LynxEngine/shaders/texture.spv"
								}));
				...
				mesh.material.setPipeline(pipelineManager->get({
							.vertMain="vertMain",
							.fragMain="fragMain",
							.vert="LynxEngine/shaders/uv.spv",
							.frag="LynxEngine/shaders/uv.spv"
							}));
			...
			{
				LYNX_ENGINE_DEBUG("Loading model: {}...", path);
				LYNX_ENGINE_DEBUG("\t Texture dir: {}", textureDir);

				tinyobj::ObjReaderConfig readerConfig;
				readerConfig.mtl_search_path = "./LynxEngine/materials";
				...
				LYNX_ENGINE_DEBUG("\tLoading {} meshes...", shapes.size());
			...
			void createDescSets()
			{
				LYNX_ENGINE_DEBUG("Creating descriptor sets...");
				...
				LYNX_ENGINE_DEBUG("Descriptor sets created!");
			}
			...
			vk::Extent2D chooseSwapExtent(const vk::SurfaceCapabilitiesKHR &cap)
			{
				...
				return {
					std::clamp<uint32_t>(size.x, cap.minImageExtent.width, cap.maxImageExtent.width),
						std::clamp<uint32_t>(size.y, cap.minImageExtent.height, cap.maxImageExtent.height)
				};
			}
			...
			void createSurface()
			{
				...
				window->createVulkanSurface(*instance, &_surface);
			...
			void pickPhysicalDevice()
			{
				...
				LYNX_ENGINE_DEBUG("Available devices:");
				for(auto &dev : devices)
				{
					LYNX_ENGINE_DEBUG((char*)(dev.getProperties().deviceName));
				}
			...
			void createInstance()
			{
				...
				{
					LYNX_ENGINE_DEBUG("Adding validation layers...");
			...
			static VKAPI_ATTR vk::Bool32 VKAPI_CALL debugCallback(vk::DebugUtilsMessageSeverityFlagBitsEXT severity, vk::DebugUtilsMessageTypeFlagsEXT type, const vk::DebugUtilsMessengerCallbackDataEXT* pCallbackData, void*) {
				LYNX_ENGINE_DEBUG("Validation layer: type {}\n\tMessage: {}", to_string(type), pCallbackData->pMessage);
			...
			void setupDebugMessenger()
			{
				...
				LYNX_ENGINE_DEBUG("Setting up the debug messenger");
